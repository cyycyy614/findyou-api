...
import com.example.demo.tools.HttpsUtil;
...
public class AppTask {
	...
	public class CurrentData {
		...
		CurrentData(String strDataAndTime, String data) {
			mStrDate = getDate(strDataAndTime);
			mStrTime = getTime(strDataAndTime);
			this.data = data;
		}
	...
	/***** Public Function *****/
	// Constructor
	public AppTask(String strIp, String strPort, String strAppId, String strPassword) throws Exception {
		// Set Data Member
	...
	public DeviceResult registerDirectDevice(String strNodeId, String strVerifyCode, int nTimeout) throws Exception {
		// Check Param
		...
		DeviceResult ret = new DeviceResult();
		// Parse Result
		Map<String, Object> mResult = new HashMap<String, Object>();
		mResult = JsonUtil.jsonString2SimpleObj(strResult, mResult.getClass());
		if (mResult.get("error_code") != null) {
			ret.code = mResult.get("error_code").toString();
			ret.message = mResult.get("error_desc").toString();
			ret.deviceId = "";
			//errorHandle(mResult);
			return ret;
		}
		ret.code = "200";
		ret.deviceId = mResult.get("deviceId").toString();
		ret.psk = mResult.get("psk").toString();
		// Return Device ID
		return ret; //mResult.get("deviceId").toString();
	}
	...
	public Map<String, Object> checkDeviceStatus(String strDeviceId) throws Exception {
		// Check Param
		if (!checkParam(strDeviceId)) {
	...
	public CurrentData queryRawData(String strDeviceId) throws Exception {
		...
		return new CurrentData(mRawDataService.get("eventTime").toString(), mRawDataService.get("data").toString());
	}
