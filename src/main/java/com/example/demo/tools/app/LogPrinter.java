package com.example.demo.tools.app;
...
public class LogPrinter {
	...
	void printDeviceBasicInfo(AppTask.DeviceBasicInfo info) {
		// Check Param
		if (info == null) {
	...
	void printCurrentData(AppTask.CurrentData objCurData, Vector<String> vPropertyList) {
		// Check Param
	...
	void printCurrentDataWithProperty(AppTask.CurrentData objCurData, String strProperty) {
		// Check Param
	...
	void printHistoryData(Map<String, Vector<AppTask.HistoryData>> mHistoryData, Vector<String> vPropertyList) {
		// Check Param
		if (vPropertyList == null || vPropertyList.size() == 0) {
			printlnAsError("No Property For This Device.");
			return;
	...
	void printHistoryDataWithProperty(Map<String, Vector<AppTask.HistoryData>> mHistoryData, String strProperty) {
		// Check Param
		...
		Vector<AppTask.HistoryData> vDataList = mHistoryData.get(strProperty);
		...
		for (int i = 0; i < vDataList.size(); ++i) {
			...
			AppTask.HistoryData objCurData = vDataList.elementAt(i);
	...
	void printAsyncCmdList(Vector<AppTask.CommandDetail> vCmdList) {
		// Check Param
		...
		for (int i = 0; i < vCmdList.size(); ++i) {
			AppTask.CommandDetail curCmd = vCmdList.elementAt(i);
	...
	// Print Rule List
	void printRuleList(Vector<AppTask.RuleInfo> vRuleList) {
		// Check Param
		...
		for (int i = 0; i < vRuleList.size(); ++i) {
			AppTask.RuleInfo info = vRuleList.elementAt(i);
